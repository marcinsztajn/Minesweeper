...
private:
	...
	void expandArea(int idx);
	// function that expands area when cell has no bomb inside
...
public:
	...
	// contains pointers to all cells on the Board
	std::vector<int>allAvailableIndices;
	std::vector<int>remainedBombs;
	std::vector<int>failedGuessIndicies;
	...
	bool checkIfVectorsContainSameIndicies(std::vector<int>a,
		std::vector<int>b);
